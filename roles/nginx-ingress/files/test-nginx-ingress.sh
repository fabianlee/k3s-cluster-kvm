#!/bin/bash
#
# Test primary and secondary LB endpoints
#
...
function call_curl() {
  ...
  #set -x
  curl --insecure --fail --resolve $domain:$port:$gateway -L -x '' $protocol://$domain:$port/$page
  retVal=$?
  #set +x
...
for domain in $gwextfqdnlist ; do
  ...
  call_curl $gwextip $domain http "myhello" 80 0
  call_curl $gwextip $domain https "myhello" 443 0
done
...
call_curl $gwextip $domain http "myhello" 80 1
call_curl $gwextip $domain https "myhello" 443 1


echo ""; echo "";
echo "***Correct Host header $domain to internal $gwintip**********************************************"
call_curl $gwintip $domain http "myhello2" 80 0
call_curl $gwintip $domain https "myhello2" 443 0


echo ""; echo "";
domain=$firstCN
echo "***Incorrect Host header $domain to internal $gwintip**********************************************"
call_curl $gwintip $domain http "myhello2" 80 1
call_curl $gwintip $domain https "myhello2" 443 1
